...
# ============================================================
# GRAFICI RISULTATI RQ1 PRINCIPALE
# ============================================================

def generate_primary_rq1_plot(results: pd.DataFrame):
    """Genera il grafico dell'analisi primaria ALL / FAKE / REAL."""
    import matplotlib.pyplot as plt

    primary = results[results["primary_analysis"]].copy()
    order = ["ALL", "FAKE", "REAL"]
    primary["scope"] = pd.Categorical(primary["scope"], categories=order, ordered=True)
    primary = primary.sort_values("scope")

    fig, ax = plt.subplots(figsize=(9, 5))
    bars = ax.barh(primary["scope"], primary["spearman_rho"])
    ax.axvline(0, linewidth=1)
    ax.set_xlim(-1, 1)
    ax.set_xlabel("Spearman correlation")
    ax.set_ylabel("Group")
    ax.set_title("RQ1 — Narrativity vs emotion strength")
    ax.grid(axis="x", alpha=0.2)

    for bar, value in zip(bars, primary["spearman_rho"]):
        x = value - 0.02 if value < 0 else value + 0.02
        ha = "right" if value < 0 else "left"
        ax.text(x, bar.get_y() + bar.get_height() / 2, f"{value:.3f}", va="center", ha=ha)

    fig.tight_layout()
    output_path = OUTPUT_DIR / "rq1_primary_correlations.png"
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)


def generate_rq1_sensitivity_plot(results: pd.DataFrame):
    """Genera un confronto delle tre definizioni di emotion strength."""
    import matplotlib.pyplot as plt
    import numpy as np

    subset = results[results["narrativity_measure"] == PRIMARY_NARRATIVITY].copy()
    scopes = ["ALL", "FAKE", "REAL"]
    measures = ["emotion_strength_max", "emotion_strength_top3_mean", "emotion_strength_rms"]
    labels = ["Max", "Top-3 mean", "RMS"]
    y = np.arange(len(scopes))
    height = 0.24

    fig, ax = plt.subplots(figsize=(10, 6))
    for offset, measure, label in zip([-height, 0, height], measures, labels):
        values = []
        for scope in scopes:
            row = subset[(subset["scope"] == scope) & (subset["emotion_strength_measure"] == measure)]
            values.append(row.iloc[0]["spearman_rho"])
    ...
    ax.set_xlabel("Spearman correlation")
    ax.set_ylabel("Group")
    ax.set_title("RQ1 — Sensitivity analysis")
    ax.legend()
    ax.grid(axis="x", alpha=0.2)
    fig.tight_layout()
    output_path = OUTPUT_DIR / "rq1_sensitivity_analysis.png"
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    # Generazione immagini dei risultati RQ1.
    generate_primary_rq1_plot(results)
    generate_rq1_sensitivity_plot(results)
