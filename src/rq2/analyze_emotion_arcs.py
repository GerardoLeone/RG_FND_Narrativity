...
# ============================================================
# BAR CHART AGGIUNTIVI RQ2
# ============================================================

def make_section_comparison_barplot(
    tests: pd.DataFrame,
    measure: str,
    output_path: Path,
):
    """Confronta le medie FAKE e REAL nelle 5 sezioni."""
    subset = tests[tests["measure"] == measure].sort_values("section")
    x = np.arange(len(subset))
    width = 0.36

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(x - width / 2, subset["fake_mean"], width=width, label="FAKE")
    ax.bar(x + width / 2, subset["real_mean"], width=width, label="REAL")
    ax.set_xticks(x)
    ax.set_xticklabels([f"Section {int(s)}" for s in subset["section"]])
    ax.set_xlabel("Normalized article section")
    ax.set_ylabel("Mean emotion strength")
    ax.set_title("RQ2 — Emotion strength by section: FAKE vs REAL")
    ax.legend()
...
def make_section_difference_barplot(
    tests: pd.DataFrame,
    measure: str,
    output_path: Path,
):
    """Mostra FAKE - REAL per ciascuna delle 5 sezioni."""
    subset = tests[tests["measure"] == measure].sort_values("section")

    fig, ax = plt.subplots(figsize=(9, 5.5))
    bars = ax.bar(subset["section"].astype(str), subset["difference_fake_minus_real"])
    ax.axhline(0, linewidth=1)
    ax.set_xlabel("Normalized article section")
    ax.set_ylabel("Mean difference (FAKE - REAL)")
    ax.set_title("RQ2 — Emotion-strength difference by section")
    ax.grid(axis="y", alpha=0.2)

    for bar, value in zip(bars, subset["difference_fake_minus_real"]):
...
def make_effect_size_barplot(
    tests: pd.DataFrame,
    measure: str,
    output_path: Path,
):
    """Mostra Cliff's delta FAKE vs REAL per ciascuna sezione."""
    subset = tests[tests["measure"] == measure].sort_values("section")

    fig, ax = plt.subplots(figsize=(9, 5.5))
    ax.bar(subset["section"].astype(str), subset["cliffs_delta_fake_vs_real"])
    ax.axhline(0, linewidth=1)
    ax.set_ylim(-1, 1)
    ax.set_xlabel("Normalized article section")
    ax.set_ylabel("Cliff's delta (FAKE vs REAL)")
    ax.set_title("RQ2 — Effect size by section")
    ax.grid(axis="y", alpha=0.2)
    fig.tight_layout()
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    # Bar chart aggiuntivi dei risultati RQ2.
    make_section_comparison_barplot(
        tests,
        "emotion_strength_max",
        OUTPUT_DIR / "rq2_section_fake_real_comparison.png",
    )
    make_section_difference_barplot(
        tests,
        "emotion_strength_max",
        OUTPUT_DIR / "rq2_section_difference.png",
    )
    make_effect_size_barplot(
        tests,
        "emotion_strength_max",
        OUTPUT_DIR / "rq2_section_effect_size.png",
    )
