...
OUTPUT_CSV = Path("../data/processed/narrativity_validation_100.csv")
...
def load_and_clean_dataset(path: Path, label: str) -> pd.DataFrame:
    ...
    print(
        f"{label}: {initial_rows} righe iniziali, "
...
def make_excerpt(text: str, position: str, length: int = 700) -> str:
    ...
    if position == "middle":
        start = max(0, len(clean) // 2 - length // 2)
...
def main():
    ...
    result["excerpt_start"] = result["text"].apply(
        lambda value: make_excerpt(value, "start")
    )
    result["excerpt_middle"] = result["text"].apply(
        lambda value: make_excerpt(value, "middle")
    )
    result["excerpt_end"] = result["text"].apply(
        lambda value: make_excerpt(value, "end")
    )
    ...
    print(f"File salvato in: {OUTPUT_CSV.resolve()}")

    print("\nStatistiche preliminari per classe:")
    print(
